...
void EngineInit(EngineState& s) {
    ...
    // DBC editor tooling. Best-effort DB connect - failure leaves the
    // browser in file-only mode.
    s.dbc_registry = std::make_unique<DbcRegistry>("assets/dbc");
    if (!s.db.Connect("db_config.toml")) {
        std::cerr << "DB connect: " << s.db.LastError() << "\n";
    }
    s.dbc_browser = std::make_unique<DbcBrowserSystem>(*s.dbc_registry, s.db);
    s.dbc_form    = std::make_unique<DbcFormSystem>(*s.dbc_registry, s.db);
    s.dbc_browser->SetFormSystem(s.dbc_form.get());
    s.icon_cache   = std::make_unique<IconCache>(*s.device, *s.imgui_ctx);
    s.spell_editor = std::make_unique<SpellEditorSystem>(s.db, *s.icon_cache);
...
void EngineFrame(EngineState& s, float dt) {
    ...
    s.dbc_browser->Update();
    s.dbc_form->Update();
    s.spell_editor->Update();
