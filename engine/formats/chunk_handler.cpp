...
ChunkId IdentifyChunk(const uint8_t* p) {
    ...
    if (p[0]==0x52 && p[1]==0x44 && p[2]==0x48 && p[3]==0x4D) return ChunkId::MHDR; // RDHM
    if (p[0]==0x58 && p[1]==0x45 && p[2]==0x54 && p[3]==0x4D) return ChunkId::MTEX; // XETM
    if (p[0]==0x58 && p[1]==0x44 && p[2]==0x4D && p[3]==0x4D) return ChunkId::MMDX; // XDMM
    if (p[0]==0x44 && p[1]==0x49 && p[2]==0x4D && p[3]==0x4D) return ChunkId::MMID; // DIMM
    if (p[0]==0x44 && p[1]==0x49 && p[2]==0x57 && p[3]==0x4D) return ChunkId::MWID; // DIWM
    if (p[0]==0x46 && p[1]==0x44 && p[2]==0x44 && p[3]==0x4D) return ChunkId::MDDF; // FDDM
    if (p[0]==0x4F && p[1]==0x32 && p[2]==0x48 && p[3]==0x4D) return ChunkId::MH2O; // O2HM
    ...
    if (p[0]==0x52 && p[1]==0x4E && p[2]==0x43 && p[3]==0x4D) return ChunkId::MCNR; // RNCM
    if (p[0]==0x56 && p[1]==0x43 && p[2]==0x43 && p[3]==0x4D) return ChunkId::MCCV; // VCCM
    if (p[0]==0x59 && p[1]==0x4C && p[2]==0x43 && p[3]==0x4D) return ChunkId::MCLY; // YLCM
    if (p[0]==0x4C && p[1]==0x41 && p[2]==0x43 && p[3]==0x4D) return ChunkId::MCAL; // LACM
...
bool WalkChunks(const uint8_t* buf, size_t total, const std::unordered_map<ChunkId, ChunkHandler>& handlers) {
    ...
    while (pos + 8 <= total) {
        ...
        auto it = handlers.find(IdentifyChunk(chunk_start));
        if (it != handlers.end()) {
