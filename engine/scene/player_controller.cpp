...
void PlayerController::Update(float dt,
    ...
    // FLATTEN forward + right onto the XZ plane so pushing W never
    // makes the player walk into the sky (or into the ground) when
    // the camera is angled. The world up is hard-coded +Y in engine
    // space.
    glm::vec3 cam_fwd = cam.GetTarget() - cam.GetPosition();
    cam_fwd.y = 0.0f;
    float flen = std::sqrt(cam_fwd.x * cam_fwd.x + cam_fwd.z * cam_fwd.z);
    if (flen < 1e-4f) {
        // Camera looking straight down - default forward to +X.
        cam_fwd = glm::vec3{1, 0, 0};
    } else {
        cam_fwd /= flen;
    }
    // Right = forward x up. Engine up = +Y, so right = (cam_fwd.z, 0, -cam_fwd.x).
